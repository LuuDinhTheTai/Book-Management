...
@Constraint(validatedBy = Read.ReadCartValidator.class)
public @interface Read {

    String message() default "Yêu cầu đọc giỏ hàng không hợp lệ";
    Class<?>[] groups() default {};
    Class<? extends Payload>[] payload() default {};

    @Component
    class ReadCartValidator implements ConstraintValidator<Read, Long> {
        ...
        public boolean isValid(Long cartId, ConstraintValidatorContext constraintValidatorContext) {
            try {
                ...
                if (!CommonUtil.hasPermission(account, Constants.PERMISSION.READ_CART)) {
                    throw new UnauthorizedAccessException("Bạn không có quyền đọc giỏ hàng");
                }
                ...
                throw new UnauthorizedAccessException("Lỗi kiểm tra quyền đọc giỏ hàng: " + e.getMessage());
            }
        }
    }
}
