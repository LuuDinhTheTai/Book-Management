...
@Constraint(validatedBy = Access.AccessAddressValidator.class)
public @interface Access {

    String message() default "Yêu cầu truy cập địa chỉ không hợp lệ";
    Class<?>[] groups() default {};
    Class<? extends Payload>[] payload() default {};

    @Component
    class AccessAddressValidator implements ConstraintValidator<Access, Long> {
        ...
        public boolean isValid(Long id, ConstraintValidatorContext context) {
            try {
                ...
                if (!CommonUtil.hasPermission(account, Constants.PERMISSION.READ_ACCOUNT)) {
                    throw new UnauthorizedAccessException("Bạn không có quyền truy cập địa chỉ");
                }
                ...
                throw new UnauthorizedAccessException("Lỗi kiểm tra quyền truy cập địa chỉ: " + e.getMessage());
            }
        }

        private boolean isOwner(Account account, Address address) {
            return address.getAccount().getUsername().equals(account.getUsername());
        }
    }
} 